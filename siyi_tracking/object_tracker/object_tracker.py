...
class MultiTrackerNode(Node):
    def __init__(self, video_path=None, playback_fps=30):
        ...
        if video_path and os.path.isfile(video_path):
            ...
            self.original_fps = 30  # Assume 30 fps for webcam
    ...
    def track_object(self):
        while rclpy.ok():
            ...
            if not ret:
                self.get_logger().warn('End of video reached or could not read frame')
                break
            ...
            if key == ord('q'):
                ...
                self.playback_fps = min(self.playback_fps + 1, self.original_fps)
                self.frame_delay = 1 / self.playback_fps
            elif key == ord('-'):
...
def main(args=None):
    ...
    video_path = input("Enter the path to the video file (or press Enter to use webcam): ").strip()
    playback_fps = float(input("Enter desired playback FPS (default is 30): ") or 30)
    multi_tracker = MultiTrackerNode(video_path if video_path else None, playback_fps)
    try:
