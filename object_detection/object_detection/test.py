...
class OffboardControl(Node):
    ...
    def calculate_direction_vector(self, current_x, current_y, current_z, target_x, target_y, target_z, speed=0.3):
    ...
    def calculate_avoidance_vector(self, current_x, current_y, target_x, target_y, speed = 0.3):
    ...
    def timer_callback(self) -> None:
        ...
        if self.offboard_setpoint_counter >= 20:
            obstacle_radius = self.distance.data
            margin = 2.5
            adjusted_distance = obstacle_radius + margin

            target_x, target_y = self.center_point.z, self.center_point.x
            ...
            if distance_to_obstacle <= adjusted_distance:
                self.get_logger().info("Obstacel Detection!! We need Avoidance!!")
                self.get_logger().info(f"{distance_to_obstacle}, {adjusted_distance}")
            elif distance_to_obstacle > adjusted_distance:
                self.get_logger().info("Now We Safe!")
                self.get_logger().info(f"{distance_to_obstacle}, {adjusted_distance}")
